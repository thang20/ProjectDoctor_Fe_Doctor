                        ...
                        Apiservice.apiservice.updateprofile(body).enqueue(new Callback<ResponseBody>() {
                            ...
                            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                                ...
                                if (notice.equals("0")) {
                                    ...
                                    if (MyApplication.getMyApplication().getDataUser().isCarepay() == false&&MyApplication.getMyApplication().getDataUser().isActivedoctor() == false) {
                                        onBackPressed();
                                    } else if(MyApplication.getMyApplication().getDataUser().isCarepay() == true&&MyApplication.getMyApplication().getDataUser().isActivedoctor() == false){
                                        startActivity(new Intent(getApplicationContext(), IDCard.class));
                                        finish();
                                    }else if(MyApplication.getMyApplication().getDataUser().isCarepay() == false&&MyApplication.getMyApplication().getDataUser().isActivedoctor() == true) {
                                        startActivity(new Intent(getApplicationContext(), IdentityActiveDoctor.class));
                                        finish();
                                        //Toast.makeText(UpdateProfile.this, "ok", Toast.LENGTH_LONG).show();
                                    }
